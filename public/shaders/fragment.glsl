...
void main() {
    //vec2 locations[2] = vec2[2](vec2(150.0, 150.0), vec2(800.0, 800.0));
    float red = 0.0;
    float green = 0.0;
    float blue = 0.0;
    float color;
    ...
    //uv = ((gl_FragCoord.x - u_location.x) * (gl_FragCoord.x - u_location.x)) + ((gl_FragCoord.y - u_location.y) * (gl_FragCoord.y - u_location.y));
    //color = 10000.0 / uv;
